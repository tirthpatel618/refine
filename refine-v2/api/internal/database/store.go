...
func (s *Store) GetGlobalLeaderboard(mode string, difficulty int) ([]models.LeaderboardEntry, error) {
	ctx, cancel := s.ctx()
	defer cancel()
	...
	rows, err := s.DB.QueryContext(ctx,
		`SELECT u.username, gs.score, gs.correct, gs.total, gs.time_limit, gs.created_at
		 ...
		 WHERE gs.mode = $1 AND gs.difficulty = $2
...
func (s *Store) GetPersonalLeaderboard(userID int64, mode string, difficulty int) ([]models.LeaderboardEntry, error) {
	ctx, cancel := s.ctx()
	defer cancel()
...
func (s *Store) GetUserStats(userID int64) ([]models.ModeStat, error) {
	ctx, cancel := s.ctx()
	defer cancel()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&s.Mode, &s.Difficulty, &s.GamesPlayed, &s.BestScore, &s.AvgScore, &s.AvgAccuracy); err != nil {
			return nil, err
		}
...
func (s *Store) GetRecentGames(userID int64) ([]models.GameSessionRecord, error) {
	ctx, cancel := s.ctx()
	defer cancel()

	rows, err := s.DB.QueryContext(ctx,
		`SELECT id, mode, difficulty, score, correct, total, time_limit, created_at
		 FROM game_sessions
		 WHERE user_id = $1
